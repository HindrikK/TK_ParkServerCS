...
namespace TK_ParkServer.Park
{
    class VestasRCS
    {
        ...
        private void AutoErrorAckTimer_Elapsed(object sender, System.EventArgs e)
        {
            ...
            if (TK_ParkServer.Settings.T01.AutoResetError.Exists(x => x == turbine.StatusCode))
            {
                try
                {
        ...
        public void OpenPort(string _PortName, int _Baudrate)
        {
            ...
            try
            {
                ...
                for (uint i = 0; i < _Park.NumOfTurbinesInPark; i++)
                {
                    CommStatus_Timer[i].Interval = TK_ParkServer.Settings.VestasDriver.CommStatusTimeout;
                }
                Poll1_Timer.Interval = TK_ParkServer.Settings.VestasDriver.PollInterval1;
                Poll2_Timer.Interval = TK_ParkServer.Settings.VestasDriver.PollInterval2;
                Poll10s_Timer.Interval = 10000;
                Timeout_Timer.Interval = TK_ParkServer.Settings.VestasDriver.CommTimeout;
                        ...
                        if (Poll2_TimerElapsed_Flag)
                        {
                            ...
                            if (errorCode_Prev != turbine.StatusCode)
                            {
                                ...
                                if (TK_ParkServer.Settings.T01.AutoResetError.Exists(x => x == turbine.StatusCode))
                                {
                                    ...
                                    {
                                        AutoErrorAck_Timer.Stop();
                                        AutoErrorAck_Timer.Interval = TK_ParkServer.Settings.T01.AutoErrorAckDelay * 1000;
