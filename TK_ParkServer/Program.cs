...
namespace TK_ParkServer
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            using (Mutex mutex = new Mutex(true, "TK_Park_Server", out createdNew))
            {
                ...
                if (createdNew)
                {
                    ...
                    #region
                    if (TK_ParkServer.Settings.Application.AutoConnectPark)
                    {
                        try
                        {
                            myVestasController.OpenPort(TK_ParkServer.Settings.VestasDriver.PortName, TK_ParkServer.Settings.VestasDriver.Baudrate);
                        }
                        catch (Exception ex)
                        {
                            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ...
                    #region
                    if (TK_ParkServer.Settings.Application.AutoStartIEC104)
                    {
                        try
                        {
                        ...
                        {
                            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            TK_ParkServer.Settings.Application.AutoStartIEC104 = false;
