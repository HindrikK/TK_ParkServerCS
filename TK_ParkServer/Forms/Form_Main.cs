...
namespace TK_ParkServer.Forms
{
    ...
    public partial class Form_Main : Form
    {
        ...
        const string PSWD_DEVELOPER = "TROONIKDeveloper";
        ...
        public Form_Main()
        {
            ...
            timer_AutoLogOff.Interval = TK_ParkServer.Settings.Application.AutoLogoutTime * 1000;
        }
        ...
        private void connectParkToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    Program.myVestasController.OpenPort(TK_ParkServer.Settings.VestasDriver.PortName, TK_ParkServer.Settings.VestasDriver.Baudrate);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(this, ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            if (!TK_ParkServer.Settings.MarketInterface.MarketIfEnable)
            {
            ...
            else
            {
                ...
                if ((DateTime.Now - marketInterfaceDownSince).TotalSeconds >= TK_ParkServer.Settings.MarketInterface.FallbackTimeout)
                {
                    ApplyMarketInterfaceFallback();
                }
