    ...
    class Alarm_Dispatch
    {
                ...
                if (IsActive)
                {
                    try
                    {
                        for (int i = 1; i <= _Park.NumOfTurbinesInPark; i++)
                        {
                            ...
                            {
                                TriggerDelay_Timer.Interval = TK_ParkServer.Settings.AlarmAnnouncement.TriggerDelay * 1000;   // update timer interval value
                            ...
                            if (TriggerDelayTimer_Flag)     // same alarm has been active long enough to trigger alarm message
                            {
                                ...
                                if (!TK_ParkServer.Settings.AlarmAnnouncement.ExcludedStatus.Exists(x => x == _Park.myTurbines[i].StatusCode))
                                {
                                    try
                                    {
        ...
        public void SendToAll(string Message)
        {
            lock (this)
            {
                ...
                if (TK_ParkServer.Settings.AlarmAnnouncement.Recipients.Count() > 0)
                {
                    for (int i = 0; i < TK_ParkServer.Settings.AlarmAnnouncement.Recipients.Count(); i++)
                    {
                        if (TK_ParkServer.Settings.AlarmAnnouncement.Recipients[i].Active)
                        {
                            System.Net.Mail.MailMessage message = new System.Net.Mail.MailMessage();
                            message.To.Add(TK_ParkServer.Settings.AlarmAnnouncement.Recipients[i].Address);
                            message.Subject = TK_ParkServer.Settings.AlarmAnnouncement.Subject;
                            message.From = new System.Net.Mail.MailAddress(TK_ParkServer.Settings.AlarmAnnouncement.FromAddress);
                            message.Body = Message;

                            AddToOutbox(message);

                            System.Net.Mail.SmtpClient smtp = new System.Net.Mail.SmtpClient(TK_ParkServer.Settings.AlarmAnnouncement.Server, TK_ParkServer.Settings.AlarmAnnouncement.Port);
                            smtp.Timeout = TK_ParkServer.Settings.AlarmAnnouncement.Timeout;
        ...
        public void SendAnnouncement(string _text)
        {
            ...
            System.Net.Mail.MailMessage message = new System.Net.Mail.MailMessage();
            message.To.Add(TK_ParkServer.Settings.AlarmAnnouncement.DefaultAddress);
